...
local lang = redis:get(hash)
		if matches[1] == 'clean' and matches[2] == 'warns' and is_mod(msg) or matches[1] == "پاک کردن" and matches[2] == 'اخطار ها' and is_mod(msg)then
    ...
    if not lang then
     ...
     return "_تمام اخطار های کاربران این گروه پاک شد_"
		...
		if matches[1] == 'setwarn' and is_mod(msg) or matches[1] == "حداکثر اخطار" and is_mod(msg) then
...
if matches[1] == "warn" and is_mod(msg) or matches[1] == "اخطار" and is_mod(msg) then
...
if matches[1] == "unwarn" and is_mod(msg) or matches[1] == "حذف اخطار" and is_mod(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
tdcli_function ({
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') and not msg.reply_id then
   tdcli_function ({
	...
	if matches[1] == "warnlist" and is_mod(msg) or matches[1] == "لیست اخطار" and is_mod(msg) then
